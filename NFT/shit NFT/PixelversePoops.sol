...
contract PixelversePoops is ERC2981, ERC721AQueryable, Ownable {
    ...
    string public _metadataURI =
        "https://lambda.pieceofshit.wtf/pixelverse-poops/metadata/";
    mapping(uint32 => uint32) public _smashTimes;

    constructor(address shit, address shitPlunger)
        ERC721A("PixelversePoops", "PP")
    {
        _shit = IERC721(shit);
    ...
    function smash(uint32[] memory poopIds, uint32[] memory smashTimes)
        external
    {
        require(_started, "PixelversePoops: Not Started");
        require(
            poopIds.length == smashTimes.length,
            "PixelversePoops: Tokens and times not match"
        );

        uint32 totalSmash = 0;
        for (uint256 i = 0; i < smashTimes.length; ) {
            require(
                _shit.ownerOf(poopIds[i]) == msg.sender,
                "PixelversePoops: You need to smash your own poops"
            );
            totalSmash += smashTimes[i];
            _smashTimes[poopIds[i]] += smashTimes[i];
            require(
                _smashTimes[poopIds[i]] <= _smashLimit,
                "PixelversePoops: Exceed smash times limit"
            );
            unchecked {
                i++;
            }
        ...
        require(
            totalSmash + _totalMinted() <= _maxSupply,
            "PixelversePoops: Exceed max supply"
        );
    ...
    function batchGetSmashTimes(uint32[] memory poopIds)
        external
        view
        returns (uint32[] memory)
    {
        uint32[] memory times = new uint32[](poopIds.length);
        for (uint256 i = 0; i < poopIds.length; i++)
            times[i] = _smashTimes[poopIds[i]];
        return times;
    }
